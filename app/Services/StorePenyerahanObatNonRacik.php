...
class StorePenyerahanObatNonRacik
{
    ...
    /**
     * POST Medication Dispense untuk Obat Non Racikan
     */
    public function handle(
        ...
        try {
            ...
            Log::info("POST MedicationDispense Non Racik Payload", $payload);

            $response = Http::withToken($token)->post($this->baseUrl, $payload);

            if ($response->failed()) {
                Log::error("Error MedicationDispense Non Racik", [
                    'body' => $response->body()
                ]);
                throw new Exception("Gagal POST MedicationDispense Non Racik: " . $response->body());
            }

            return $response->json('id');

        } catch (\Exception $e) {
            Log::error("StorePenyerahanObatNonRacik Error: " . $e->getMessage());
            throw $e;
        }
    }
