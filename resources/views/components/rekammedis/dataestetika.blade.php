<div class="bg-base-200 p-4 rounded border border-base-200">
    ...
    <div class="space-y-4">
            ...
            <div class="flex flex-wrap gap-4">
                @foreach (['Jerawat', 'Bekas Jerawat', 'Kulit Kering', 'Kulit Kasar', 'Kulit Kusam', 'Kulit Berminyak', 'Keriput', 'Pori-pori Besar', 'Flek', 'Stres/Kelelahan'] as $problem)
                    <label class="flex items-center gap-2">
                        <input type="checkbox" wire:model="data_estetika.problem_dihadapi" value="{{ $problem }}" class="checkbox checkbox-sm" />
                        <span>{{ $problem }}</span>
                    </label>
                @endforeach
            </div>
        ...
        <div>
            ...
            <input id="lama_problem" type="text" class="input input-bordered w-full" wire:model.defer="data_estetika.lama_problem">
        </div>
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-4">
                @foreach (['Didiamkan', 'Diobati Sendiri', 'Ke Dokter', 'Ke Salon'] as $tindakan)
                    <label class="flex items-center gap-2">
                        <input type="checkbox" wire:model="data_estetika.tindakan_sebelumnya" value="{{ $tindakan }}" class="checkbox checkbox-sm" />
                        <span>{{ $tindakan }}</span>
                    </label>
                @endforeach
            </div>
        ...
        <div>
            ...
            <input id="penyakit_dialami" type="text" class="input input-bordered w-full" wire:model.defer="data_estetika.penyakit_dialami">
        </div>
        ...
        <div>
            ...
            <input id="alergi_kosmetik" type="text" class="input input-bordered w-full" wire:model.defer="data_estetika.alergi_kosmetik">
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            ...
            <div id="usia_kehamilan_section" style="display: none;">
                ...
                <input id="usia_kehamilan" type="number" min="1" class="input input-bordered w-full" wire:model.defer="data_estetika.usia_kehamilan">
            </div>
        </div>
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-4">
                @foreach (['Pil', 'Suntik', 'Implant', 'IUD', 'Steril', 'Tidak Menggunakan'] as $kb)
                    <label class="flex items-center gap-2">
                        <input type="radio" wire:model="data_estetika.metode_kb" value="{{ $kb }}" class="radio radio-sm" />
                        <span>{{ $kb }}</span>
                    </label>
                @endforeach
            </div>
        ...
        <div>
            ...
            <input id="pengobatan_saat_ini" type="text" class="input input-bordered w-full" wire:model.defer="data_estetika.pengobatan_saat_ini">
        </div>

        {{-- Produk Kosmetik --}}
        <div>
            <label for="produk_kosmetik" class="label"><span class="label-text">Produk Kosmetik yang Sedang Dipakai</span></label>
            <input id="produk_kosmetik" type="text" class="input input-bordered w-full" wire:model.defer="data_estetika.produk_kosmetik">
        </div>
