        ...
        <div class="card bg-base-100 shadow-md border border-info/50">
            <div class="card-body">
                ...
                <div class="relative w-full h-[120px] sm:h-[160px]">
                    <canvas wire:ignore class="w-full" id="grafikKunjunganBulananBar"></canvas>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const ctxKunjunganBulananBar = document.getElementById('grafikKunjunganBulananBar');

        const dataKunjunganBulananBar = new Chart(ctxKunjunganBulananBar, {
            type: 'bar',
            data: {
                labels: [],
                datasets: []
        ...
        Livewire.on('update-kunjungan-bulanan-bar', data => {
            const payload = data[0];

            dataKunjunganBulananBar.data.labels = payload.labelsBulan;
            dataKunjunganBulananBar.data.datasets.length = 0;
            payload.datasets.forEach(ds => {
                dataKunjunganBulananBar.data.datasets.push(ds);
            });

            dataKunjunganBulananBar.update();
        });
    });
</script>