            ...
            <div class="card bg-base-100 shadow-md border border-info/50">
                <div class="card-body">
                    ...
                    <div wire:loading.remove wire:target="tahunDipilih,resetData" class="relative w-full h-[120px] sm:h-[160px]">
                        <canvas id="grafikKunjunganBulananBar" class="absolute inset-0 w-full h-full"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataKunjunganBulananBar = null;

        Livewire.on('update-kunjungan-bulanan-bar', data => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikKunjunganBulananBar');
            if (!ctxBar) return;

            if (dataKunjunganBulananBar) dataKunjunganBulananBar.destroy();

            dataKunjunganBulananBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
