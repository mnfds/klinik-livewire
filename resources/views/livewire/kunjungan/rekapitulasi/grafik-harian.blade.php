...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataKunjunganHarianBar = null;
        let dataKunjunganHarianPie = null;

        Livewire.on('update-kunjungan-harian-bar', (data) => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikKunjunganHarianBar');
            if (!ctxBar) return;

            if (dataKunjunganHarianBar) dataKunjunganHarianBar.destroy();

            dataKunjunganHarianBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
        ...
        Livewire.on('update-kunjungan-harian-pie', (data) => {
            const payload = data[0];
            const ctxPie = document.getElementById('grafikKunjunganHarianPie');
            if (!ctxPie) return;

            if (dataKunjunganHarianPie) dataKunjunganHarianPie.destroy();

            dataKunjunganHarianPie = new Chart(ctxPie, {
                type: 'pie',
                data: {
