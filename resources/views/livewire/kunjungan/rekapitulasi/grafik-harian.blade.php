    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-4"> 
        ...
        <div class="card bg-base-100 shadow-md border border-success/50 lg:col-span-3">
            <div class="card-body">
                <h3 class="text-sm font-semibold mb-2 flex items-center gap-2">
                    ...
                    Grafik Kunjungan Pasien Harian
                </h3>
                <canvas id="grafikKunjunganHarianBar" class="w-full h-[260px] sm:h-[320px]"></canvas>
            </div>
        </div>
        ...
        <div class="card bg-base-100 shadow-md border border-error/50">
            <div class="card-body flex flex-col items-center justify-center">
                ...
                <canvas id="grafikKunjunganHarianPie" class="w-[180px] h-[180px] sm:w-[220px] sm:h-[220px]"></canvas>
            </div>
        </div>
    </div>
</div>
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const ctxKunjunganHarianBar = document.getElementById('grafikKunjunganHarianBar');
        const dataKunjunganHarianBar = new Chart(ctxKunjunganHarianBar, {
            type: 'bar',
            data: {
        ...
        Livewire.on('update-kunjungan-harian-bar', (data) => {
            const payload = data[0];
            dataKunjunganHarianBar.data.labels = payload.labelstanggal;
            dataKunjunganHarianBar.data.datasets = payload.datasets;

            dataKunjunganHarianBar.update();
        });
    });

    document.addEventListener('DOMContentLoaded', function () {
        const ctx = document.getElementById('grafikKunjunganHarianPie');

        const chartKunjunganPie = new Chart(ctx, {
            type: 'pie',
            data: {
                labels: [],
                datasets: [{
        ...
        Livewire.on('update-kunjungan-harian-pie', payload => {
            const data = payload[0]; // 🔥 INI KUNCINYA

            console.log('DATA POLI KUNJUNGAN LAKI PIE:', data);
            chartKunjunganPie.data.labels = data.labels;
            chartKunjunganPie.data.datasets[0].data =
                data.datasets[0].data;
            chartKunjunganPie.data.datasets[0].backgroundColor =
                data.datasets[0].backgroundColor;
            chartKunjunganPie.data.datasets[0].borderColor =
                data.datasets[0].borderColor;

            chartKunjunganPie.update();
        });
