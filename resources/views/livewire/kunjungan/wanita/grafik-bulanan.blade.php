            ...
            <div class="card bg-base-100 shadow-md border border-info/50">
                <div class="card-body">
                    ...
                    <div wire:loading.remove wire:target="tahunDipilih,resetData" class="relative w-full h-[120px] sm:h-[160px]">
                        <canvas id="grafikKunjunganWanitaBulananBar" class="absolute inset-0 w-full h-full"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataKunjunganWanitaBulananBar = null;

        Livewire.on('update-kunjungan-wanita-bulanan-bar', data => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikKunjunganWanitaBulananBar');
            if (!ctxBar) return;

            if (dataKunjunganWanitaBulananBar) dataKunjunganWanitaBulananBar.destroy();

            dataKunjunganWanitaBulananBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
