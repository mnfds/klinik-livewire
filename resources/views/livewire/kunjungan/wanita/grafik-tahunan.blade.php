            ...
            <div class="card bg-base-100 shadow-md border border-warning/50">
                <div class="card-body">
                    ...
                    <div class="relative w-full h-[120px] sm:h-[160px]">
                        <canvas id="grafikKunjunganWanitaTahunanBar" class="absolute inset-0 w-full h-full"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataKunjunganWanitaTahunanBar = null;

        Livewire.on('update-kunjungan-wanita-tahunan-bar', data => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikKunjunganWanitaTahunanBar');
            if (!ctxBar) return;

            if (dataKunjunganWanitaTahunanBar) dataKunjunganWanitaTahunanBar.destroy();

            dataKunjunganWanitaTahunanBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
