        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div class="card bg-base-100 shadow-md border border-success/50 lg:col-span-3">
                <div class="card-body">
                    ...
                    <canvas wire:loading.remove wire:target="tanggalDipilih,resetData"
                        id="grafikKunjunganWanitaHarianBar"
            ...
            <div class="card bg-base-100 shadow-md border border-error/50">
                <div class="card-body flex flex-col items-center justify-center">
                    ...
                    <canvas wire:loading.remove wire:target="tanggalDipilih,resetData"
                        id="grafikKunjunganWanitaHarianPie"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataKunjunganWanitaHarianBar = null;
        let dataKunjunganWanitaHarianPie = null;

        Livewire.on('update-kunjungan-wanita-harian-bar', (data) => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikKunjunganWanitaHarianBar');
            if (!ctxBar) return;

            if (dataKunjunganWanitaHarianBar) dataKunjunganWanitaHarianBar.destroy();

            dataKunjunganWanitaHarianBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
        ...
        Livewire.on('update-kunjungan-wanita-harian-pie', (data) => {
            const payload = data[0];
            const ctxPie = document.getElementById('grafikKunjunganWanitaHarianPie');
            if (!ctxPie) return;

            if (dataKunjunganWanitaHarianPie) dataKunjunganWanitaHarianPie.destroy();

            dataKunjunganWanitaHarianPie = new Chart(ctxPie, {
                type: 'pie',
                data: {
