            ...
            <div class="card bg-base-100 shadow-md border border-info/50">
                <div class="card-body">
                    ...
                    <div wire:loading.remove wire:target="tahunDipilih,resetData" class="relative w-full h-[120px] sm:h-[160px]">
                        <canvas id="grafikKunjunganLakiBulananBar" class="absolute inset-0 w-full h-full"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataKunjunganLakiBulananBar = null;

        Livewire.on('update-kunjungan-laki-bulanan-bar', data => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikKunjunganLakiBulananBar');
            if (!ctxBar) return;

            if (dataKunjunganLakiBulananBar) dataKunjunganLakiBulananBar.destroy();

            dataKunjunganLakiBulananBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
