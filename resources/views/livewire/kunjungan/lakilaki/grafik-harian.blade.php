        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div class="card bg-base-100 shadow-md border border-success/50 lg:col-span-3">
                <div class="card-body">
                    ...
                    <canvas wire:loading.remove wire:target="tanggalDipilih,resetData"
                        id="grafikKunjunganLakiHarianBar"
            ...
            <div class="card bg-base-100 shadow-md border border-error/50">
                <div class="card-body flex flex-col items-center justify-center">
                    ...
                    <canvas wire:loading.remove wire:target="tanggalDipilih,resetData"
                        id="grafikKunjunganLakiHarianPie"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataKunjunganLakiHarianBar = null;
        let dataKunjunganLakiHarianPie = null;

        Livewire.on('update-kunjungan-laki-harian-bar', (data) => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikKunjunganLakiHarianBar');
            if (!ctxBar) return;

            if (dataKunjunganLakiHarianBar) dataKunjunganLakiHarianBar.destroy();

            dataKunjunganLakiHarianBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
        ...
        Livewire.on('update-kunjungan-laki-harian-pie', (data) => {
            const payload = data[0];
            const ctxPie = document.getElementById('grafikKunjunganLakiHarianPie');
            if (!ctxPie) return;

            if (dataKunjunganLakiHarianPie) dataKunjunganLakiHarianPie.destroy();

            dataKunjunganLakiHarianPie = new Chart(ctxPie, {
                type: 'pie',
                data: {
