<div wire:init="loadGrafik" class="mt-4">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between bg-base-100 p-4 rounded-lg shadow-sm border border-primary/50">
                ...
                <div class="flex flex-col gap-2 w-full lg:w-auto lg:items-end">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                        <button type="button" wire:click="openFilterModal" onclick="document.getElementById('applyFilterModalTahunan').showModal()" class="btn btn-primary btn-sm">
        ...
        <div class="grid grid-cols-1 gap-4">
            <div class="card bg-base-100 shadow-md border border-warning/50">
                <div class="card-body">
                    ...
                    {{-- Spinner saat filter diterapkan --}}
                    <div wire:loading wire:target="filter,resetAll" class="flex items-center justify-center h-[120px] sm:h-[160px]">
                        <span class="loading loading-spinner loading-md text-warning"></span>
                    </div>
                    <div wire:loading.remove wire:target="filter,resetAll" class="relative w-full h-[120px] sm:h-[160px]">
                        <canvas id="grafikRekapTahunanBar" class="absolute inset-0 w-full h-full"></canvas>
                    </div>
                </div>
            </div>
        </div>
    ...
    {{-- MODAL FILTER --}}
    <dialog id="applyFilterModalTahunan" class="modal" wire:ignore.self>
        <div class="modal-box w-full max-w-xl">
            <h3 class="text-xl font-semibold mb-4">Filter Grafik Tahunan</h3>

            <form x-on:submit.prevent="await $wire.filter(); document.getElementById('applyFilterModalTahunan').close();" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="col-span-1 sm:col-span-2 flex justify-end gap-2 mt-4">
                    ...
                    <button type="button" class="btn btn-neutral" onclick="document.getElementById('applyFilterModalTahunan').close()">Batal</button>
                </div>
            </form>
        </div>
    </dialog>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataRekapTahunanBar = null;

        Livewire.on('update-rekap-tahunan-bar', data => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikRekapTahunanBar');
            if (!ctxBar) return;
