            ...
            <div class="card bg-base-100 shadow-md border border-warning/50">
                <div class="card-body">
                    ...
                    <div class="relative w-full h-[120px] sm:h-[160px]">
                        <canvas id="grafikApotikTahunanBar" class="absolute inset-0 w-full h-full"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataApotikTahunanBar = null;

        Livewire.on('update-apotik-tahunan-bar', data => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikApotikTahunanBar');
            if (!ctxBar) return;

            if (dataApotikTahunanBar) dataApotikTahunanBar.destroy();

            dataApotikTahunanBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
                    ...
                    datasets: [
                        {
                            label: 'Pendapatan',
                            data: payload.apotikTahunanBarMasuk,
                        ...
                        {
                            label: 'Pengeluaran',
                            data: payload.apotikTahunanBarKeluar,
