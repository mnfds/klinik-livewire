            ...
            <div class="card bg-base-100 shadow-md border border-info/50">
                <div class="card-body">
                    ...
                    <div wire:loading.remove wire:target="tahunDipilih,resetData" class="relative w-full h-[120px] sm:h-[160px]">
                        <canvas id="grafikApotikBulananBar" class="absolute inset-0 w-full h-full"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataApotikBulananBar = null;

        Livewire.on('update-apotik-bulanan-bar', data => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikApotikBulananBar');
            if (!ctxBar) return;

            if (dataApotikBulananBar) dataApotikBulananBar.destroy();

            dataApotikBulananBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
                    ...
                    datasets: [
                        {
                            label: 'Pendapatan',
                            data: payload.apotikBulananBarMasuk,
                        ...
                        {
                            label: 'Pengeluaran',
                            data: payload.apotikBulananBarKeluar,
