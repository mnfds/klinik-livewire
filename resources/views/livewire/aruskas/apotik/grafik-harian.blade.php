...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataApotikHarianBar = null;
        let dataApotikHarianPie = null;

        Livewire.on('update-apotik-harian-bar', (data) => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikApotikHarianBar');
            if (!ctxBar) return;

            if (dataApotikHarianBar) dataApotikHarianBar.destroy();

            dataApotikHarianBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
                    ...
                    datasets: [
                        {
                            label: 'Pendapatan',
                            data: payload.apotikHarianBarMasuk,
                        ...
                        {
                            label: 'Pengeluaran',
                            data: payload.apotikHarianBarKeluar,
        ...
        Livewire.on('update-apotik-harian-pie', (data) => {
            const payload = data[0];
            const ctxPie = document.getElementById('grafikApotikHarianPie');
            if (!ctxPie) return;

            if (dataApotikHarianPie) dataApotikHarianPie.destroy();

            dataApotikHarianPie = new Chart(ctxPie, {
                type: 'pie',
                data: {
                    labels: ['Pendapatan', 'Pengeluaran'],
                    datasets: [{
                        data: [payload.apotikHarianPieMasuk, payload.apotikHarianPieKeluar],
                        backgroundColor: [
                            'rgba(34,197,94,0.6)',
                            'rgba(239,68,68,0.6)'
                        ],
