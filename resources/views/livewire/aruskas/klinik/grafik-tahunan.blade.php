            ...
            <div class="card bg-base-100 shadow-md border border-warning/50">
                <div class="card-body">
                    ...
                    <div class="relative w-full h-[120px] sm:h-[160px]">
                        <canvas id="grafikKlinikTahunanBar" class="absolute inset-0 w-full h-full"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let dataKlinikTahunanBar = null;

        Livewire.on('update-klinik-tahunan-bar', data => {
            const payload = data[0];
            const ctxBar = document.getElementById('grafikKlinikTahunanBar');
            if (!ctxBar) return;

            if (dataKlinikTahunanBar) dataKlinikTahunanBar.destroy();

            dataKlinikTahunanBar = new Chart(ctxBar, {
                type: 'bar',
                data: {
                    ...
                    datasets: [
                        {
                            label: 'Pendapatan',
                            data: payload.klinikTahunanBarMasuk,
                        ...
                        {
                            label: 'Pengeluaran',
                            data: payload.klinikTahunanBarKeluar,
